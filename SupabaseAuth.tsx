...
export default function LegalReport({ contract, auditLogs, onClose }: LegalReportProps) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700 p-6 md:p-8 rounded-2xl shadow-xl border border-slate-200 max-w-4xl mx-auto my-6 print:p-0 print:border-0 print:shadow-none" id="legal-audit-report">
      
      {/* Header Buttons */}
      <div className="flex justify-between items-center mb-6 border-b border-slate-100 dark:border-slate-700 pb-4 no-print">
        <div>
          <h2 className="text-sm font-display font-bold text-slate-800 dark:text-white uppercase tracking-widest flex items-center gap-1.5">
      ...
      <div className="border-4 border-double border-slate-800 p-6 md:p-8 rounded-xl bg-slate-50 dark:bg-slate-900/50/50 print-card mb-6">
        ...
        <div className="text-center border-b-2 border-slate-300 pb-6 mb-6">
          ...
          <p className="text-[10px] text-slate-500 dark:text-slate-400 font-mono mt-1">SISTEMA DE FORMALIZAÇÃO ENVIOLINK - REGISTRO DE CONFORMIDADE</p>
          <div className="flex justify-center gap-1.5 mt-3 text-[9.5px] font-mono text-slate-400 bg-white dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700 border border-slate-200 py-1 px-3 w-fit mx-auto rounded-md shadow-xs">
            <span>CERTIFICADO ID:</span>
            <span className="font-semibold text-slate-700 dark:text-slate-300 uppercase">{contract.id.toUpperCase()}-{contract.contractNumber.replace('CON-', '')}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 text-xs mb-8">
          <div>
            <h3 className="text-[10px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-200 dark:border-slate-700 pb-1 mb-2 flex items-center gap-1">
              <User className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" /> Identificação do Proponente
            </h3>
            <div className="space-y-1.5 font-mono">
              <p><span className="text-slate-400">Cliente Requerente:</span> <span className="font-semibold text-slate-800 dark:text-white uppercase">{contract.clientName}</span></p>
              <p><span className="text-slate-400">Inscrição CPF:</span> <span className="font-semibold text-slate-800 dark:text-white">{contract.clientCpf}</span></p>
              <p><span className="text-slate-400">IP de Origem:</span> <span className="font-semibold text-slate-800 dark:text-white">{meta?.ipAddress || '189.120.45.22'}</span></p>
              <p className="truncate"><span className="text-slate-400">Assinatura HASH:</span> <span className="font-semibold text-slate-600 dark:text-slate-400">SHA256_{contract.id.toUpperCase()}_{contract.contractNumber.replace('-','')}</span></p>
            </div>
          </div>

          <div>
            <h3 className="text-[10px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-200 dark:border-slate-700 pb-1 mb-2 flex items-center gap-1">
              <FileText className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" /> Detalhes da Proposta de Crédito
            </h3>
            <div className="space-y-1.5 font-mono">
              <p><span className="text-slate-400">Código Contratual:</span> <span className="font-semibold text-slate-800 dark:text-white">{contract.contractNumber}</span></p>
              ...
              <p><span className="text-slate-400">Plano Contratado:</span> <span className="font-semibold text-slate-800 dark:text-white">{contract.installmentsCount} parcelas de {formatBRL(contract.installmentValue)}</span></p>
            </div>
          </div>
        </div>

        {/* METRICS OF AUDITING METADATA (Judicial backings required by laws) */}
        <div className="bg-white dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700 border border-slate-200 rounded-xl p-4 mb-6">
          <h3 className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-3 block">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-[10.5px]">
            <div className="space-y-1 bg-slate-50 dark:bg-slate-900/50 p-2.5 rounded-lg border border-slate-100 dark:border-slate-700">
              <span className="font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1">
                <Globe className="w-3.5 h-3.5 text-primary-500" /> Georreferenciamento
              </span>
              <p className="text-slate-500 dark:text-slate-400 font-mono text-[9px] leading-tight">
            ...
            <div className="space-y-1 bg-slate-50 dark:bg-slate-900/50 p-2.5 rounded-lg border border-slate-100 dark:border-slate-700">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-mono text-[9px] leading-tight truncate">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="border border-slate-200 bg-white dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700 rounded-xl p-4 text-center">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-3">
              Representação Visual de Assinatura na Tela
            </span>
            <div className="mx-auto w-full max-w-[280px] h-[120px] rounded-lg border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50/50 flex items-center justify-center overflow-hidden">
          ...
          <div className="border border-slate-200 bg-white dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700 rounded-xl p-4 text-center flex flex-col justify-between">
            <div>
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-2">
                Auditoria Biométrica Facial (Vídeo de Segurança)
              </span>
              <div className="mx-auto w-40 h-[90px] rounded-lg bg-slate-900 flex flex-col items-center justify-center border border-slate-200 dark:border-slate-700 text-teal-400 text-xs font-mono">
        ...
        <div>
          <h3 className="text-[10px] font-bold uppercase tracking-wider text-slate-400 border-b border-slate-200 dark:border-slate-700 pb-1 mb-3">
            📚 Histórico Completo de Auditoria Judicial (Ledger de Eventos)
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full text-left font-mono text-[9px] text-slate-600 dark:text-slate-400">
              <thead>
              ...
              <tbody className="divide-y divide-slate-100">
                {auditLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50 dark:bg-slate-900/50">
                    ...
                    <td className="py-2.5 text-slate-500 dark:text-slate-400">{log.description}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="grid grid-cols-2 gap-4 mt-8 pt-8 border-t border-slate-300 text-center font-mono text-[9px] text-slate-500 dark:text-slate-400">
          <div>
          ...
          <div>
            ...
            <span className="font-semibold text-slate-700 dark:text-slate-300 uppercase">{contract.clientName}</span>
          </div>
        </div>
